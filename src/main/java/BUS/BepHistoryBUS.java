 ...
 */
@Path("/")
public class BepHistoryBUS {
    @GET
    @Path("{bephistoryID}")
    @Produces(MediaType.APPLICATION_JSON)
    
    public Response getBepHistoryById(@PathParam("bephistoryID") int bephistory_id) {
        BepHistoryDAO dao = new BepHistoryDAO();
        BepHistoryDTO dto = dao.getBepHistoryById(bephistory_id);
        if(dto != null) {
    ...
    public Response addBepHistory(BepHistoryDTO bephistoryJSON) {
        BepHistoryDAO dao = new BepHistoryDAO();
        //if(dao.checkBepHistoryEmail(bephistoryJSON.getEmail())) {
            int result = dao.addBepHistory(bephistoryJSON);
            if(result != -1) {
...
//        }
    ...
    public Response updateBepHistory(@PathParam("bephistoryID") int bephistory_id, BepHistoryDTO bephistoryJSON) {
        BepHistoryDAO dao = new BepHistoryDAO();
        BepHistoryDTO dto = dao.getBepHistoryById(bephistory_id);
        bephistoryJSON.setBepHistoryId(bephistory_id);
        if(dto != null) {
//            if(dto.getUsername().equals(bephistoryJSON.getUsername())) {
                boolean result = dao.updateBepHistory(bephistoryJSON);
...
//    @Produces(MediaType.APPLICATION_JSON)
    ...
    public Response deleteBepHistoryById(@PathParam("bephistoryID") int bephistory_id) {
        BepHistoryDAO dao = new BepHistoryDAO();
        BepHistoryDTO dto = dao.getBepHistoryById(bephistory_id);
        if(dto != null) {
            boolean result = dao.deleteBepHistory(bephistory_id);
            if(result) {
                return Response.ok().header("Access-Control-Allow-Origin", "*")
                .header("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT")
